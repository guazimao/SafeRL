...
def plot_data(data, xaxis='Epoch', value="EpRet/Mean", 
...
def get_datasets(logdir, condition=None):
    ...
    for root, _, files in os.walk(logdir):
        if 'progress.txt' in files:
            ...
            try:
                exp_data = pd.read_table(os.path.join(root,'progress.txt'), sep=' ')
            except:
                print('Could not read from %s'%os.path.join(root,'progress.txt'))
                continue
            print(exp_data)
            performance = 'AverageTestEpRet' if 'AverageTestEpRet' in exp_data else 'EpRet/Mean'
...
def main():
    ...
    Args: 
        ...
        value (strings): Pick what columns from data to graph on the y-axis. 
            ...
            Instead, ``Performance`` refers to either ``EpRet/Mean``, the 
            correct performance measure for the on-policy algorithms, or
            ``AverageTestEpRet``, the correct performance measure for the 
            off-policy algorithms. The plotter will automatically figure out 
            which of ``EpRet/Mean`` or ``AverageTestEpRet`` to report for 
